...
export default function Suscriptions() {
  ...
  const fetchStores = async () => {
    try {
      ...
      setStores(response.data.stores); // Almacenar las tiendas en el estadoo
    } catch (error) {
  ...
  const subscribeToStore = async (storeId) => {
    try {
      ...
      // Después de suscribirse, actualiza el estado local

      setStores((prevStores) => {
        return prevStores.map((store) =>
          store.id === storeId
            ? { ...store, isSubscribed: true } // Aquí marcaríamos la tienda como suscrita
            : store
        );
      });

      // Si prefieres obtener las suscripciones actualizadas desde el servidor
      // fetchStores();
    } catch (error) {
  ...
  const unsubscribeFromStore = async (storeId) => {
    try {
      ...
      // Después de desuscribirse, actualiza el estado local

      setStores((prevStores) => {
        return prevStores.map((store) =>
          store.id === storeId
            ? { ...store, isSubscribed: false } // Aquí marcaríamos la tienda como desuscrita
            : store
        );
      });
