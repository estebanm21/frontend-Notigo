...
//     // const getMySuscriptions = async () => {
//     //     const url = "http://192.168.1.40:3000/api/v1/suscription/mysubscriptions"
//     //     try {
...
//                                 </Text>
//                             </View>
//                             <TouchableOpacity onPress={() => unsubscribeFromStore(store.id)} style={styles.suscriptionButton}>
...
export default function CardStore({ item: store, index, calculateDistance, formatDistance, filteredStores, location, unsubscribeFromStore, isSubscribed, subscribeToStore }) {

    const [suscription, setSuscription] = useState();
    const [isMenuOpen, setIsMenuOpen] = useState(false); // Para controlar la visibilidad del menú
    const [isSubscribedState, setIsSubscribedState] = useState(isSubscribed); // Estado de la suscripción
        ...
        const checkSubscription = async () => {
            try {
                ...
                if (parsedSubscriptions.includes(store.id)) {
                    setIsSubscribedState(true); // El usuario está suscrito
                } else {
                    setIsSubscribedState(false); // El usuario no está suscrito
                }
            } catch (error) {
    ...
    const handleSubscriptionChange = async () => {
        try {
            ...
            setIsSubscribedState(!isSubscribedState); // Actualizar el estado local
                ...
                <View style={[styles.card_store, isLastItem && styles.lastCard]}>
                    <View>
                        ...
                        <View style={styles.container_info}>
                            ...
                            <TouchableOpacity
                                style={[styles.suscriptionButton, isSubscribedState ? styles.suscribed : styles.notSuscribed]}
                                onPress={() => setIsMenuOpen(!isMenuOpen)}
                            >
                                <BellIcon size={20} color={isSubscribedState ? "#FFD700" : "#ccc"} />
                                <Text style={styles.suscriptionButtonText}>
                                    {isSubscribedState ? i18n.t('subscribed') : i18n.t('subscribe')}
                                </Text>
                                ...
                                <View style={styles.menuOptions}>
                                    ...
                                    >
                                        <Text style={styles.menuOptionText}>
                                            {isSubscribedState ? i18n.t('unsubscribe') : i18n.t('subscribe')}
                                        </Text>
                                    </TouchableOpacity>
                                </View>
                            )}
